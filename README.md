...
这是深知写作助手的 GitHub 分发版本。功能逻辑与自用版保持一致，但不内置深知搜索 API Key；首次调用本 Skill 时，无论当前任务是否需要搜索，都先由 Agent 通过 MaaS 注册接口完成手机号注册、验证码确认、API Key 获取和本地配置写入，用户只需提供手机号和收到的验证码。
...
GitHub 版的初始化与具体任务无关：只要调用本 Skill 且本地没有 `config.ini`，Agent 就应先引导用户完成注册配置，然后再继续处理原任务。简单通知、改写润色、只生成 Word 等场景也遵循该规则。
...
- 接入点 `type=6`，即深知可信搜索。
- 注册请求不传 `channel` 字段，由 MaaS 使用默认渠道。
...
如自动注册链路失败，可降级使用 GitHub 默认渠道注册链接手动注册：
